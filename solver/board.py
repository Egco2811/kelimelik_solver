...
class ScrabbleBoard:
    ...
    def _is_out_of_bounds(self, r, c):
        return r < 0 or r >= SIZE or c < 0 or c >= SIZE

    def _is_boundary_clear(self, board, r, c):
        if self._is_out_of_bounds(r, c):
            return True
        return board[r][c] is None

    def _check_word_boundaries(self, word, sr, sc, dr, dc, board):
        br = sr - dr
        bc = sc - dc
        if not self._is_boundary_clear(board, br, bc):
            return False
        er = sr + len(word)*dr
        ec = sc + len(word)*dc
        if not self._is_boundary_clear(board, er, ec):
            return False
        return True
    ...
    def _validate_and_score(self, prefix, sr, sc, dr, dc, new_mask, used, new_cells, dictionary):
        ...
        if not self._check_word_boundaries(prefix, sr, sc, dr, dc, temp_board):
            return None
            ...
            if is_new:
                ...
                if len(cross_word) > 1:
                    if not dictionary.is_word(cross_word):
                        valid = False
                        break
                    if not self._check_word_boundaries(cross_word, cross_sr, cross_sc, perp_dr, perp_dc, temp_board):
                        valid = False
                        break
                    cross_new_mask = []
                    cross_joker = []
    ...
    def generate_moves(self, rack, dictionary):
        ...
        def dfs(cr, cc, prefix, rack_left, used, new_mask, new_cells,
                sr, sc, dr, dc, dir_name, anchor_index):
            nonlocal start_time
            if time.time() - start_time > TIMEOUT:
                raise TimeoutError

            if used and len(prefix) > anchor_index and dictionary.is_word(prefix):
                score = self._validate_and_score(prefix, sr, sc, dr, dc, new_mask, used, new_cells, dictionary)
            ...
            if not self.is_empty(cr, cc):
                ...
                if dictionary.is_prefix(prefix + lt):
                    dfs(cr+dr, cc+dc, prefix+lt, rack_left, used,
                        new_mask+[False], new_cells+[(cr,cc)],
                        sr, sc, dr, dc, dir_name, anchor_index)
            else:
                children = dictionary.get_children(prefix)
                for i, tile in enumerate(rack_left):
                    if tile == '?':
                        letters_to_try = children if children else set()
                    else:
                        letters_to_try = [tile] if tile in children else []
                ...
                for dr,dc,dir_name in DIRS:
                    ...
                    for left_len in range(max_left+1):
                        ...
                        dfs(start_r, start_c, '', rack_letters, [], [], [],
                            start_r, start_c, dr, dc, dir_name, anchor_index)
        except TimeoutError:
            pass
